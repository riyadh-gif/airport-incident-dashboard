<x-app-layout>
    <div class="mx-auto max-w-4xl space-y-5">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ __('Add flights') }}</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ __('Add one or more flight rows, then submit them all at once.') }}</p>
        </div>

        <div class="card p-6 sm:p-8"
             x-data="{
                 rows: [{ flight_no: '', occurred_at: '{{ now()->toDateString() }}', location: '' }],
                 add() { this.rows.push({ flight_no: '', occurred_at: '{{ now()->toDateString() }}', location: '' }); },
                    ...
                    <template x-for="(row, i) in rows" :key="i">
                        <div class="relative rounded-2xl border border-slate-200 bg-slate-50/50 p-4 transition dark:border-slate-800 dark:bg-slate-800/30">
                            ...
                            <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
                                <div>
                                    <label class="form-label">{{ __('Flight no.') }}</label>
